...
Accepts PDF uploads via WebSocket
"""
...
def load_pdf_from_bytes(pdf_bytes):
    ...
    global PDF_DOCUMENT, TOTAL_SLIDES, CURRENT_SLIDE

    try:
        PDF_DOCUMENT = fitz.open(stream=pdf_bytes, filetype="pdf")
        TOTAL_SLIDES = len(PDF_DOCUMENT)
        CURRENT_SLIDE = 0
        print(f"PDF loaded from upload")
...
async def handle_viewer_client(websocket):
    ...
    try:
        # Send initial slide if PDF is loaded
        if PDF_DOCUMENT is not None:
            await broadcast_slide_update()
        else:
            # Send a message indicating no PDF is loaded
            await websocket.send(json.dumps({
                'type': 'no_pdf',
                'message': 'No PDF loaded. Please upload a PDF file.'
            }))
                ...
                if command == 'upload_pdf':
                    ...
                    if pdf_base64:
                        try:
                            ...
                            if load_pdf_from_bytes(pdf_bytes):
                                await websocket.send(json.dumps({
                                    'type': 'upload_success',
                                    'message': f'PDF loaded successfully. {TOTAL_SLIDES} slides.'
                                }))
                                # Broadcast first slide to all clients
                                await broadcast_slide_update()
                            else:
                                await websocket.send(json.dumps({
                                    'type': 'upload_error',
...
async def main():
    ...
    try:
        async with websockets.serve(route_connection, host, port):
            print("PDF Server: Ready to accept connections...\n")
            print("Waiting for PDF upload from web interface...\n")
